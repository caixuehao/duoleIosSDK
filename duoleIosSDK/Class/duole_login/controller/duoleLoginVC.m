...
-(void)loadSecondMainView{
    ...
    SecondMainView.alpha = 0.01;
    SecondMainView.backgroundColor = ColorRGBA(0, 0, 0, 0.6);
    [[SecondMainView layer]setCornerRadius:10.0];//圆角
...
-(void)loadSecondMainViewSubviews:(NSInteger)Type;{
    ...
    [btn.layer setCornerRadius:5.0];//圆角大小
    btn.showsTouchWhenHighlighted = YES; //按下发光
    btn.titleLabel.font = [UIFont fontWithName:@"Helvetica-Bold" size:20];//字体大小
    btn.frame = CGRectMake(20, (45*titleText_arr.count)+45, 210, 45);
...
-(void)loadMainViewSubviews{
    ...
    //临时账号提示
    UILabel *messageLabel = [[UILabel alloc] initWithFrame:CGRectMake(20, (60*2+fisrtBtnH), 210, 50)];
